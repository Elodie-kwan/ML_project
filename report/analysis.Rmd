...
unique.mountain <- unique(Mountain_data_cleaned)
set.seed(123) ## for replication purpose
## the index of the rows that will be in the training set
index.tr <- sample(1:nrow(unique.mountain), replace=FALSE,
                   size=0.75*nrow(unique.mountain))

Mountain_data.tr <- unique.mountain[index.tr,] ## the training set
Mountain_data.te <- unique.mountain[-index.tr,] ## the test set
unique.mountain <- unique(Mountain_data_cleaned)

KNN <- knn3(data=Mountain_data.tr,Mountain_data.tr$Mountain_range ~ ., K=3)


predict(KNN, newdata = Mountain_data.te, type = "class")
